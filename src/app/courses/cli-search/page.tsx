...
export default function CLISearchCoursePage() {
  return (
    <div className="container mx-auto py-8 px-4">
      {/* Course Header */}
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-4">{cliSearchCourse.title}</h1>
        <p className="text-lg text-muted-foreground mb-4">
          {cliSearchCourse.description}
        </p>
        <div className="flex gap-4 items-center flex-wrap">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium">Duration:</span>
            <span className="text-sm text-muted-foreground">
              {cliSearchCourse.duration / 60} hours
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium">Level:</span>
            <span className="text-sm text-muted-foreground capitalize">
              {cliSearchCourse.level}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium">Total XP:</span>
            <span className="text-sm text-muted-foreground font-bold">
              {cliSearchCourse.totalXP}
            </span>
          </div>
        </div>
      </div>
      ...
      <Card className="p-6 mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {cliSearchCourse.levelThresholds
      ...
      <div className="space-y-8">
        {cliSearchCourse.phases.map((phase) => {
                ...
                {phase.modules.map((module) => {
                  ...
                  return (
                    <Card key={module.id} className="p-6 hover:shadow-lg transition-shadow">
                      ...
                      <Link href={`/courses/cli-search/${module.id}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-3xl font-bold text-primary">
              {cliSearchCourse.phases.length}
            </div>
            <div className="text-sm text-muted-foreground">Phases</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-primary">
              {cliSearchCourse.phases.reduce(
