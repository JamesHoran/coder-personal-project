...
export default function JestCoursePage() {
  return (
    <div className="container mx-auto py-8 px-4">
      {/* Course Header */}
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-4">{jestCourse.title}</h1>
        <p className="text-lg text-muted-foreground mb-4">
          {jestCourse.description}
        </p>
        <div className="flex gap-4 items-center flex-wrap">
          <div className="flex items-center gap-2">
            <span className="text-sm font-medium">Duration:</span>
            <span className="text-sm text-muted-foreground">
              {jestCourse.duration / 60} hours
            </span>
          </div>
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground font-bold">
              {jestCourse.totalXP.toLocaleString()}
            </span>
          </div>
        </div>
      </div>
                ...
                {phase.modules.map((module) => {
                  ...
                  return (
                    <Card key={module.id} className="p-6 hover:shadow-lg transition-shadow">
                      ...
                      <Link href={`/courses/jest/${module.id}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-3xl font-bold text-primary">
              {jestCourse.phases.length}
            </div>
            <div className="text-sm text-muted-foreground">Phases</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-primary">
              {jestCourse.phases.reduce(
